...
import {
  CardBrandLogo, CardPanel, ErrorMsg, FIELD, Field, InfoMsg, InstallmentField,
  MethodRadio, PIX_MINUTES, PixPanel, cardBrand, installmentSub, maskCEP,
  onlyDigits, type InstallmentOption,
    ...
    <section id="step-2" className={`bg-white rounded-2xl shadow-sm scroll-mt-20 ${!active ? "opacity-50 pointer-events-none" : ""}`}>
      ...
      {active && (
        <div className="px-5 pb-5">
          <div className="space-y-2">
            {/* Roteiro só-WhatsApp pode liberar o PIX à vista sem liberar o cartão:
                PIX custa R$ 1,89 fixo, cartão custa percentual. Quem quer parcelar
                continua indo ao WhatsApp. */}
            {(!booking.trip_whatsapp_only || booking.trip_allow_pix) && (
          ...
          <div key={method} className="mt-5 animate-pop">
            {method === "pix" && <PixPanel base={`/payments/${code}`} amount={booking.final_amount} onConfirmed={onConfirmed} pollStatus={pollStatus} />}
            {method === "card" && <CardPanel base={`/payments/${code}`} amount={booking.final_amount} options={booking.installment_options || []} installments={installments} setInstallments={setInstallments} onConfirmed={onConfirmed} />}

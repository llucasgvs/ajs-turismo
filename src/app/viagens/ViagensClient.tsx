...
export default function ViagensClient({ initialTemplates }: { initialTemplates: PublicTemplate[] }) {
  ...
  // Atualiza em background quando o usuário navega de volta à página (stale-while-revalidate manual)
  useEffect(() => {
            ...
            {showWhenPicker && (
              <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.15)] border border-gray-100 z-50 p-4 sm:left-0 sm:right-auto sm:w-80">
                <div className="flex bg-gray-100 rounded-xl p-1 mb-4">
                  <button onClick={() => setCalViewMode("datas")} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-[color,background-color,border-color,box-shadow,transform,opacity] ${calViewMode === "datas" ? "bg-white text-navy-900 shadow-sm" : "text-gray-500"}`}>Datas</button>
                  <button onClick={() => setCalViewMode("meses")} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-[color,background-color,border-color,box-shadow,transform,opacity] ${calViewMode === "meses" ? "bg-white text-navy-900 shadow-sm" : "text-gray-500"}`}>Meses</button>
                </div>
                      ...
                      {getCalendarDays(calMonth.year, calMonth.month).map((day, i) => {
                        ...
                        return (
                          <button key={i} disabled={!hasTrips}
                            ...
                            className={`relative flex flex-col items-center justify-center rounded-lg aspect-square text-xs font-semibold transition-[color,background-color,border-color,box-shadow,transform,opacity] duration-150 ${
                    ...
                    {nextMonths.map(({ key, label }) => {
                      ...
                      return (
                        <button key={key} disabled={count === 0}
                          ...
                          className={`flex flex-col items-start px-3 py-2.5 rounded-xl border text-left transition-[color,background-color,border-color,box-shadow,transform,opacity] ${
        ...
        <div className="flex items-center mb-4">
          <div className="relative" ref={sortRef}>
            <button onClick={() => setShowSortMenu(!showSortMenu)}
              className="flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-medium border bg-white text-gray-600 border-gray-200 hover:border-gray-300 hover:shadow-sm transition-[color,background-color,border-color,box-shadow,transform,opacity] whitespace-nowrap">
            ...
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="h-8 skeleton rounded-xl w-full mt-4" />
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="bg-navy-900 py-16 px-4">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <a href={WA_URL} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-3 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-lg px-8 py-4 rounded-2xl transition-[color,background-color,border-color,box-shadow,transform,opacity] duration-200 hover:scale-105 hover:shadow-xl shadow-emerald-500/30">
